...
[EVENT_TYPES.JACKPOT]: { name: "JACKPOT 777", desc: "RANGE: -5000 ~ 5000 PTS", color: "#facc15", shadow: "rgba(250, 204, 21, 0.8)", bgGradient: "from-yellow-500/40 to-purple-900/40" },
...
setIsFinished(true);
setTimeout(() => { if (onCompleteRef.current) onCompleteRef.current(); }, 2000);
}
};
frameRef.current = requestAnimationFrame(animate);
...
// ★修正: ここにあった「オフラインゲスト自動削除 (kickOfflineUsers)」を削除しました。
// これにより、プレイ中の再接続が可能になります。
...
} else if (eventType === EVENT_TYPES.JACKPOT) addScore = jackpotAmount || 1000;
...
nextPlayer.candidates = shuffleArray([mainChallenge, subChallenge1, subChallenge2]);
} else {
...
{currentEventData && (
<motion.div initial={{ y: -20, opacity: 0, scale: 1.2 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ scale: 0, opacity: 0 }} className="w-full mb-2 flex flex-col items-center justify-center relative">
<div className={`absolute inset-0 blur-xl opacity-40 bg-gradient-to-r ${currentEventData.bgGradient} rounded-full`}></div>
<motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="relative z-10 px-4 py-1 border-y border-white/20 bg-black/60 backdrop-blur-md" style={{ boxShadow: `0 0 20px ${currentEventData.shadow}` }}>
<p className="text-[8px] font-mono tracking-[0.3em] text-white font-bold">EVENT</p>
<h2 className="text-xl md:text-5xl font-black italic tracking-tighter" style={{ color: currentEventData.color, textShadow: `0 0 10px ${currentEventData.shadow}` }}>{currentEventData.name}</h2>
...
{isAnyoneSelectingMission ? "CHOOSING MISSION..." : isAnyoneSelectingTarget ? "CHOOSING TARGET..." : isAnyoneSelectingPartner ? "CHOOSING PARTNER..." : currentChallenge.title}
</h1>
</div>
...
<p className="font-black text-white tracking-widest text-[clamp(1.2rem,4vw,3rem)]">{isInteractionLocked ? "???" : currentChallenge.criteria}</p>
</div>
</div>
</motion.div>
)}
...
<div className={`w-full h-full flex items-center justify-center border rounded-xl backdrop-blur-md ${isAnyoneSelectingTarget ? 'bg-emerald-900/20 border-emerald-500/30' : isAnyoneSelectingPartner ? 'bg-cyan-900/20 border-cyan-500/30' : 'bg-yellow-900/20 border-yellow-500/30'}`}>
<p className={`${isAnyoneSelectingTarget ? 'text-emerald-400' : isAnyoneSelectingPartner ? 'text-cyan-400' : 'text-yellow-400'} font-bold animate-pulse tracking-widest flex items-center gap-2`}>
<span className={`w-2 h-2 rounded-full ${isAnyoneSelectingTarget ? 'bg-emerald-400' : isAnyoneSelectingPartner ? 'bg-cyan-400' : 'bg-yellow-400'}`}></span>
{showJackpot ? "JACKPOT TIME!!" : isAnyoneSelectingTarget ? "PLAYER IS CHOOSING TARGET..." : isAnyoneSelectingPartner ? "PLAYER IS CHOOSING PARTNER..." : "PLAYER IS CHOOSING MISSION..."}
</p>
</div>
)}
</div>
