...
export const FreeModeScreen = () => {
  ...
  // --- 即時退出の検知 (Button Leave対応) ---
  useEffect(() => {
    if (prevMembersRef.current.length > 0 && members.length > 0) {
      // 以前はいたけど今いない人（退出者）
      const leavers = prevMembersRef.current.filter(p => !members.find(m => m.id === p.id));
      leavers.forEach(leaver => {
        if (leaver.id !== userId) {
           addToast(`${leaver.name} が退出しました`);
        }
      });
      
      // 今いるけど以前いなかった人（参加者）
      const joiners = members.filter(m => !prevMembersRef.current.find(p => p.id === m.id));
      joiners.forEach(joiner => {
         if (joiner.id !== userId) {
           addToast(`${joiner.name} が参加しました`);
         }
      });
    }
    prevMembersRef.current = members;
  }, [members, userId, addToast]);
  ...
  const confirmBackToLobby = async () => {
    try {
      ...
      // ★修正: ホストはReady維持、ゲストのみ解除
      const resetMembers = members.map(m => {
        if (m.isHost) {
             return { ...m, isReady: true }; // ホストはTrueのまま
        }
        return { ...m, isReady: false }; // ゲストはFalse
      });
